...
def llm_year_analysis():
    ...
    if data and 'yearAnalysisResponse' in data:
        prompt = [f"You are an expert stock day trader. Here was your previous yearly analysis last month on the stock market: {data['yearAnalysisResponse']}. This table is the bar candle chart for the past year with a month long timeframe for each candle for all the stocks worth following . Please make analysises on these stocks: ", imageYear]
    else:
        prompt = [f"You are an expert stock day trader. This table is the bar candle chart for the past year with a month long timeframe for each candle for all the stocks worth following . Please make analysises on these stocks: ", imageYear]
...
def llm_month_analysis():
    ...
    if data and 'yearAnalysisResponse' in data:
        ...
        prompt = [f"You are an expert stock day trader. This table is the bar candle chart for the past month with a week long timeframe  for each candle for all the stocks worth following . Please make analysises on these stocks as well as compare the performance to the year long table from before: ", imageMonth]
    response = AgentModel.generate_content(prompt, generation_config=config)
    dictionary = {
...
def llm_week_analysis():
    ...
    if data and 'yearAnalysisResponse' in data and 'weekAnalysisResponse' in data and 'monthAnalysisResponse' in data:
        ...
        prompt = [f"You are an expert stock day trader. This table is the bar candle chart for the past week with a day long timeframe  for each candle for all the stocks worth following . Please make analysises on these stocks as well as compare the performance to the month long table from before: ", imageWeek]
    response = AgentModel.generate_content(prompt, generation_config=config)
    dictionary = {
...
def createPlan():
    ...
    if int(buyingPower) <= 0:
        prompt = [f"""You are an expert stock day trader. You make trades every 30 minutes to maximize profits. You have used up all your buying power and can only sell. Consider your current portfolio as well: {portfolioState} and consider your open orders: {orderState}. After viewing the yearly, monthly, weekly, and the day bar charts earlier in our context and previous messages keep them in mind and let's move on to market decisions.
        You currently have numerous tools at your disposal such as "get_specific_ticker_symbol_market_snapshots" to assess what actions you want to take between selling, or holding current positions. I highly recommend you use the tool.
        Make a decision on what stocks to sell for the next 30 minutes or to hold your current positions. Please be very specific in the actions you want to take including price to sell, quantity to sell, etc."""]
    elif portfolioState == []:
        prompt = [f"""You are an expert stock day trader. You make trades every 30 minutes to maximize profits. You can only buy. Consider your current portfolio as well: {portfolioState} and consider your open orders: {orderState}. After viewing the yearly, monthly, weekly, and the day bar charts earlier in our context and previous messages keep them in mind and let's move on to market decisions.
        You currently have numerous tools at your disposal such as "get_specific_ticker_symbol_market_snapshots" to assess what actions you want to take between buying, or holding current positions. I highly recommend you use the tool.
        Make a decision on what stocks to buy for the next 30 minutes or to hold your current positions. Please be very specific in the actions you want to take including price to buy, quantity to buy, etc."""]
    else:
        prompt = [f"""You are an expert stock day trader. Here is your current buying power in cash: {buyingPower}. Consider your current portfolio: {portfolioState} and consider your open orders: {orderState}. You make trades every 30 minutes to maximize profits. After viewing the yearly, monthly, weekly, and the day bar charts earlier in our context and previous messages keep them in mind and let's move on to market decisions.
        You currently have numerous tools at your disposal such as "get_specific_ticker_symbol_market_snapshots" to assess what actions you want to take between buying, selling, or holding current positions. I highly recommend you use the tool.
        Make a decision on what stocks to buy and sell for the next 30 minutes or to hold your current positions. Please be very specific in the actions you want to take including price to buy/sell, quantity to buy/sell, etc."""]
